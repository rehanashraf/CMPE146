...
public:
	 ...
	 */
	bool writeReg(uint8_t deviceAddress, uint8_t registerAddress,
			uint8_t value);
	...
	uint8_t getregDataIndex(void); // returns the slave mode recieved first byte this will be the address

	uint8_t isThereDataToRead(void); //check if there is any data that we can read.

	/**
	 * Initializes I2C slave device
	 * @param slaveAddr  The slave device address
	 * @param buffer  Point to slave read/write buffer
	 * @param size  The slave buffer size
	 */
	bool initSlave(const uint8_t slaveAddr, uint8_t *buffer, uint8_t size); // Slave initialization
...
protected:
	 ...
	 */
	WEAK bool init(uint32_t pclk, uint32_t busRateInKhz);
...
private:
	 ...
	 */
	typedef enum {
		busy, readComplete, writeComplete
	...
	/**
	 * Structure for i2c slave transactions
	 *
	 */
	typedef struct {
		uint8_t *pSlaveData;    ///< I2C slave read/write buffer
		uint8_t slaveCount;    ///< Amount of available buffer space

		// ### This should not be changed after initSlave() ###
		uint8_t trxSize;        ///< I2C slave read/write buffer size
		uint8_t slaveAddr;      ///< Slave device address
		uint8_t *pBaseSlaveData; ///< Base pointer for pSlave Data (copy of original)

		// Is <<ONLY CHANGED>> on slave-mode receiver first byte
		uint8_t regDataIndex;  ///< The read/write index in pSlaveData
	} sI2CTransaction_t;

	bool gotAddrData = false;
	 ...
	 *              - Read  is complete
	 */
	WEAK mStateMachineStatus_t i2cStateMachine();
	 ...
	 */
	void i2cKickOffTransfer(uint8_t devAddr, uint8_t regStart, uint8_t* pBytes,
			uint32_t len);
